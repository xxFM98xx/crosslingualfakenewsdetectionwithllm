...
def data2gpu(batch, use_cuda, data_type, model_name):
    if use_cuda:
        if data_type == 'rationale' and model_name == 'ARG':
            batch_data = {
        ...
        elif data_type == 'rationale' and model_name.startswith('RoBERTa'):
            if len(batch) == 8:
                batch_data = {
                    'content': batch[0].cuda(),
                    'content_masks': batch[1].cuda(),
                    'td_rationale': batch[2].cuda(),
                    'td_rationale_masks': batch[3].cuda(),
                    'cs_rationale': batch[4].cuda(),
                    'cs_rationale_masks': batch[5].cuda(),
                    'label': batch[6].cuda(),
                    'id': batch[7].cuda(),
                }
            else:
            ...
            print('Error: Unsupported data type or model name!')
            exit()
    else:
        if data_type == 'rationale' and model_name == 'ARG':
            batch_data = {
            ...
            if len(batch) == 8:
                batch_data = {
                    'content': batch[0],
                    'content_masks': batch[1],
                    'td_rationale': batch[2],
                    'td_rationale_masks': batch[3],
                    'cs_rationale': batch[4],
                    'cs_rationale_masks': batch[5],
                    'label': batch[6],
                    'id': batch[7],
                }
            else:
